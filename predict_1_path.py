...
def predictint(imvalues):
    ...
    for imvalue in imvalues:
        ...
        file, imvalue = imvalue 

        with tf.Session() as sess:
            sess.run(init_op)
            saver.restore(sess, "model.ckpt")
            prediction=tf.argmax(y,1)
            value = prediction.eval(feed_dict={x: [imvalue]}, session=sess)
            result = file,value[0]
...
def main(argv):
    ...
    for file in files:
        fullpath = argv + file    
        imvalue = imageprepare(fullpath)
        imvalues.append((file, imvalue))

    predint = predictint(imvalues)
    
    for result in predint:
        file_name, predict_value = result
        text_split = file_name.split('.')
        file_name = text_split[-2]
        print("file_name : " + file_name + " predict: " + str(predict_value))
